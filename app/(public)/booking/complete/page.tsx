      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
        ...
        {reservation && (
          <div className="bg-white rounded-xl shadow p-6 mb-6">
            ...
            <dl className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><dt className="text-gray-500">予約番号</dt><dd className="font-medium text-xs break-all">{reservationId?.slice(0, 8)}</dd></div>
            </dl>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl shadow p-6 mb-6">
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3"><span className="w-6 h-6 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">1</span><div><p className="font-medium">受付のQRコードを読み取り</p><p className="text-sm text-gray-600">受付に設置されたQRコードをスマートフォンで読み取ってください</p></div></div>
            <div className="flex items-start gap-3"><span className="w-6 h-6 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">2</span><div><p className="font-medium">チェックイン情報入力</p><p className="text-sm text-gray-600">お名前・生年月日・電話番号を入力し、保険証を撮影してください</p></div></div>
            <div className="flex items-start gap-3"><span className="w-6 h-6 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">3</span><div><p className="font-medium">待合室でお待ちください</p><p className="text-sm text-gray-600">お名前をお呼びします</p></div></div>
          </div>
        </div>
