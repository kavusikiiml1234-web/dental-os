...
function BookingFormContent() {
  ...
  function validate(): boolean {
    ...
    if (phone && !/^[0-9-]+$/.test(phone)) newErrors.phone = '電話番号の形式が正しくありません'
      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-center gap-2 mb-6">
          ...
          <div className="flex items-center">
            ...
            <span className="ml-2 text-sm font-medium text-blue-600">情報入力</span>
          </div>
        ...
        <div className="bg-blue-50 rounded-lg p-4 mb-6">
          <p className="text-sm text-blue-800">
            <span className="font-medium">ご予約日時：</span>
            {date && format(new Date(date + 'T00:00:00'), 'yyyy年M月d日(E)', { locale: ja })} {time}
          </p>
          <Link href="/booking" className="text-sm text-blue-600 hover:underline">
            ← 日時を変更する
          </Link>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input
                ...
                className={`w-full border rounded-lg px-4 py-3 ${errors.nameLast ? 'border-red-500' : 'border-gray-300'}`}
                placeholder="山田"
              />
              {errors.nameLast && <p className="text-red-500 text-xs mt-1">{errors.nameLast}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <input
                ...
                className={`w-full border rounded-lg px-4 py-3 ${errors.nameFirst ? 'border-red-500' : 'border-gray-300'}`}
                placeholder="太郎"
              />
              {errors.nameFirst && <p className="text-red-500 text-xs mt-1">{errors.nameFirst}</p>}
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className={`w-full border rounded-lg px-4 py-3 ${errors.phone ? 'border-red-500' : 'border-gray-300'}`}
              placeholder="090-1234-5678"
            />
            {errors.phone && <p className="text-red-500 text-xs mt-1">{errors.phone}</p>}
            <p className="text-xs text-gray-500 mt-1">※ 予約確認のご連絡に使用します</p>
          </div>
          ...
          <div className="mb-4">
            ...
            />
            <p className="text-xs text-gray-500 mt-1">※ リマインドメールをお送りします</p>
          </div>

          {/* 診療内容 */}
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">診療内容</label>
            <select
              value={category}
            ...
            >
              ...
              <option value="emergency">急患・痛みがある</option>
              <option value="other">その他</option>
            </select>
          </div>
          ...
          {/* 備考 */}
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-1">ご要望・症状など</label>
            <textarea
              value={note}
              onChange={(e) => setNote(e.target.value)}
              className="w-full border border-gray-300 rounded-lg px-4 py-3"
              rows={3}
              placeholder="気になる症状や、ご希望があればご記入ください"
            />
          </div>
