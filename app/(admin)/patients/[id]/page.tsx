...
function InsuranceTab({ patientId, insurances, onUpdate }: { patientId: string, insurances: any[], onUpdate: () => void }) {
  ...
  async function handleOCR() {
    ...
    if (!latestInsurance?.insurance_card_front) {
      setOcrError('保険証画像がありません')
      return
    }
    ...
    try {
      const response = await fetch('/api/ocr', {
        ...
        body: JSON.stringify({ image: latestInsurance.insurance_card_front }),
      })
      ...
      if (result.success && result.data) {
        // フォームに自動入力
        setForm({
          insurer_number: result.data.insurer_number || form.insurer_number,
          insurer_name: result.data.insurer_name || form.insurer_name,
          symbol: result.data.symbol || form.symbol,
          insured_number: result.data.insured_number || form.insured_number,
          insured_name: result.data.insured_name || form.insured_name,
          relationship: result.data.relationship || form.relationship,
          copay_rate: result.data.copay_rate?.toString() || form.copay_rate,
          valid_from: result.data.valid_from || form.valid_from,
          valid_until: result.data.valid_until || form.valid_until,
        })
        setEditing(true) // 編集モードに切り替え
      } else {
        setOcrError(result.error || 'AI読み取りに失敗しました')
      }
    } catch (error: any) {
      console.error('OCR error:', error)
      setOcrError('AI読み取り中にエラーが発生しました')
    }
