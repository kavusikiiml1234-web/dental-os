...
export default function PatientsPage() {
  ...
  useEffect(() => { fetchPatients() }, [sortField, sortOrder, currentPage])
  ...
  async function searchPatients() {
    ...
    const { data, count } = await supabase
      .from('patients')
      .select('*', { count: 'exact' })
      .or(`phone.ilike.%${searchQuery}%,name_last.ilike.%${searchQuery}%,name_first.ilike.%${searchQuery}%,name_last_kana.ilike.%${searchQuery}%,name_first_kana.ilike.%${searchQuery}%,patient_number.ilike.%${searchQuery}%`)
      .eq('is_active', true)
      .order(sortField, { ascending: sortOrder === 'asc' })
      .limit(PAGE_SIZE)
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">全 {totalCount.toLocaleString()} 名</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow p-4 mb-6">
        <form onSubmit={(e) => { e.preventDefault(); searchPatients() }} className="flex gap-4">
          ...
          {searchQuery && (
            <button 
              type="button" 
              onClick={() => { setSearchQuery(''); setCurrentPage(1); fetchPatients() }} 
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">フリガナ</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {patients.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-blue-600">{p.patient_number || '-'}</td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{p.phone || '-'}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{genderLabels[p.gender || ''] || '-'}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{p.birth_date || '-'}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                      {format(new Date(p.created_at), 'yyyy/MM/dd')}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm space-x-2">
                      <Link href={`/charts/${p.id}`} className="inline-flex items-center px-2 py-1 bg-green-50 text-green-700 rounded hover:bg-green-100">
                        📋 カルテ
                      </Link>
            ...
            {totalPages > 1 && (
              <div className="px-6 py-4 border-t flex items-center justify-between">
                <p className="text-sm text-gray-500">
                  {((currentPage - 1) * PAGE_SIZE + 1).toLocaleString()} - {Math.min(currentPage * PAGE_SIZE, totalCount).toLocaleString()} / {totalCount.toLocaleString()} 件
                </p>
